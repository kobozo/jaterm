...
pub async fn deploy_public_key(
    ...
    ssh_sftp_mkdirs(state.clone(), session_id.clone(), "~/.ssh".to_string())
        .await
    ...
    if check_auth_keys.stdout.trim() == "missing" {
        ...
        if create_auth_keys.exit_code != 0 {
            return Err(format!(
                "Failed to create authorized_keys: {}",
                create_auth_keys.stderr
            ));
        }
    }
    ...
    let check_key = ssh_exec(
        ...
        session_id.clone(),
        format!(
            "grep -F '{}' ~/.ssh/authorized_keys >/dev/null 2>&1 && echo exists || echo missing",
    ...
    if append_key.exit_code != 0 {
        return Err(format!(
            "Failed to append key to authorized_keys: {}",
            append_key.stderr
        ));
    }
    ...
    if chmod_auth_keys.exit_code != 0 {
        eprintln!(
            "Warning: chmod 600 authorized_keys failed: {}",
            chmod_auth_keys.stderr
        );
    }
...
pub async fn test_key_auth(
    ...
    use crate::commands::ssh::{SshAuth, SshProfile};

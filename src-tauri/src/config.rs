...
pub fn state_file_path(app_name: Option<&str>) -> Result<PathBuf> { Ok(ensure_config_dir(app_name)?.join("state.json")) }
pub fn profiles_file_path(app_name: Option<&str>) -> Result<PathBuf> { Ok(ensure_config_dir(app_name)?.join("profiles.json")) }
pub fn config_file_path(app_name: Option<&str>) -> Result<PathBuf> { Ok(ensure_config_dir(app_name)?.join("config.json")) }
...
fn get_schema_version(cfg: &Value) -> u32 {
  cfg.get("schemaVersion").and_then(|v| v.as_u64()).map(|v| v as u32).unwrap_or(0)
}

fn set_schema_version(cfg: &mut Value, v: u32) {
  match cfg {
    Value::Object(map) => { map.insert("schemaVersion".to_string(), Value::from(v)); },
    _ => {
...
fn migration_1_split_files(app_name: Option<&str>) -> Result<()> {
  ...
  if profiles_path.exists() {
    if !config_path.exists() { json_write_atomic(&config_path, &serde_json::json!({}))?; }
    if !new_state_path.exists() { json_write_atomic(&new_state_path, &serde_json::json!({}))?; }
    return Ok(());
  }

  // No legacy file: initialize empty files
  if !legacy_path.exists() {
    if !profiles_path.exists() { json_write_atomic(&profiles_path, &serde_json::json!({}))?; }
    if !config_path.exists() { json_write_atomic(&config_path, &serde_json::json!({}))?; }
    if !new_state_path.exists() { json_write_atomic(&new_state_path, &serde_json::json!({}))?; }
    return Ok(());
  }
  ...
  let profiles_val = {
    let mut m = serde_json::Map::new();
    if let Some(v) = legacy_obj.remove("profiles") { m.insert("profiles".to_string(), v); }
    if let Some(v) = legacy_obj.remove("profilesTree") { m.insert("profilesTree".to_string(), v); }
    Value::Object(m)
  };

  let state_val = {
    let mut m = serde_json::Map::new();
    for k in ["recents","recentSessions","recentSshSessions","lastOpenedPath","workspace"] { if let Some(v) = legacy_obj.remove(k) { m.insert(k.to_string(), v); } }
    Value::Object(m)
  };

  // Merge any unknown keys back into state to avoid data loss
  if let Value::Object(mut s_obj) = state_val.clone() {
    for (k, v) in legacy_obj.into_iter() { s_obj.insert(k, v); }
